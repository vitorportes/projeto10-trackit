...
const History = () => {
  ...
  useEffect(() => {
    setDateHabits(
      habitsHistory.find(
        (habit) => habit.day === date.toLocaleDateString("pt-br", dateOptions)
      )
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [date]);
        ...
        tileClassName={({ date }) => {
          const history = habitsHistory.map((day) => {
            return {
              day: day.day,
              allDone:
                day.habits.filter((habit) => habit.done).length ===
                day.habits.length,
            };
          });
          ...
          if (
            ...
            if (
              allDones.includes(date.toLocaleDateString("pt-br", dateOptions))
            ) {
              return "react-calendar__tile--all-done";
            } else if (
              notAllDone.includes(date.toLocaleDateString("pt-br", dateOptions))
            ) {
        ...
        {dateHabits ? (
          <>
            ...
            {dateHabits.habits.map((habit) => (
              <TodayHabitContainer className="date-habit" key={habit.id}>
